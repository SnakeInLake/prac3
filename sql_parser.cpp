...
bool executeDelete(const string& sql_query) {
    ...
    // Временный двусвязный список для хранения всех строк, которые нужно перенести после удаления
    ...
    file_number = 1;
    csv_filename = base_csv_filename + to_string(file_number) + ".csv";
    int new_pk = 1;  // Новый порядковый номер для строк
    ...
    while (current_node != nullptr) {
        ...
        output_csv << new_pk << "," << current_node->data.substr(current_node->data.find(',') + 1) << endl;
        new_pk++;
        current_row_count++;
...
while (current_row_left != nullptr) {
    ...
    if (column == "*") {
        ...
        while (fs::exists(order_table_path)) {
            ...
            cout <<"ВЫВОД КОНЧИЛСЯ "<<endl;
            order_file.close();
            break;
        }
                    cout <<"ВЫВОД КОНЧИЛСЯ2 "<<endl;
